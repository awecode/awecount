...
class SalesVoucherViewSet(InputChoiceMixin, DeleteRows, CRULViewSet):
    ...
    search_fields = ['voucher_no', 'party__name', 'remarks', 'total_amount', 'party__tax_registration_number',
                     'customer_name',
                     'date', 'address', 'rows__item__name', 'mode', 'bank_account__bank_name',
    ...
    def get_queryset(self, **kwargs):
        ...
        if self.action == 'retrieve':
            ...
            qs = qs.select_related('party').prefetch_related(
                Prefetch('payment_receipts', PaymentReceipt.objects.exclude(status='Cancelled'), to_attr='receipts'))
    ...
    def details(self, request, pk):
        qs = super().get_queryset().prefetch_related(
            Prefetch('rows',
                     SalesVoucherRow.objects.all().select_related('item', 'unit', 'discount_obj',
                                                                  'tax_scheme').order_by('pk'))).prefetch_related(
            Prefetch('payment_receipts', PaymentReceipt.objects.exclude(status='Cancelled'),
                     to_attr='receipts')).select_related(
        ...
        data['available_bank_accounts'] = GenericSerializer(BankAccount.objects.filter(company=request.company),
                                                            many=True).data
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not request.query_params.get("fifo_inconsistency"):
            if request.company.inventory_setting.enable_fifo:
                if PurchaseVoucherRow.objects.filter(voucher__date__gt=date, item__in=item_ids).exists():
        ...
        if self.is_filtered_by_date():
            ...
            qs = super().get_queryset().filter(company_id=self.request.company_id,
                                               status__in=['Issued', 'Paid', 'Partially Paid',
                                                           'Cancelled']).select_related(
                'discount_obj', 'party').order_by('date', 'voucher_no')
            ...
            years = [ad2bs(self.request.query_params.get('start_date'))[0],
                     ad2bs(self.request.query_params.get('end_date'))[0]]
            years.sort()
            year_str = '/'.join(set([str(year) for year in years]))

            ws.cell(column=1, row=4,
                    value='करदाता दर्ता नं (PAN) : {}        करदाताको नाम: {}         साल    {}      कर अवधि: {} -{}'.format(
                        self.request.company.tax_registration_number, self.request.company.name,
                        year_str, ad2bs_str(self.request.query_params.get('start_date')).replace('-', '.'),
...
class PurchaseBookViewSet(CompanyViewSetMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def export(self, request):
        if self.is_filtered_by_date():
            ...
            ws = wb.active
            qs = super().get_queryset().filter(company_id=self.request.company_id,
                                               status__in=['Issued', 'Paid', 'Partially Paid']).select_related(
                'discount_obj', 'party').order_by('date', 'voucher_no')
            queryset = self.filter_queryset(qs).annotate(total_quantity=Sum('rows__quantity'))
...
class PaymentReceiptViewSet(CRULViewSet):
    ...
    def collection_report(self, request):
        ...
        values = included.distinct().values('invoices__sales_agent_id', 'invoices__sales_agent__name').annotate(
