...
# from awecounting.libs.db import DistinctSum
from awecounting.libs import get_next_voucher_no
from awecounting.libs.CustomViewSet import (
    CollectionViewSet,
    CompanyViewSetMixin,
    CRULViewSet,
    GenericSerializer,
)
from awecounting.libs.exception import UnprocessableException
from awecounting.libs.mixins import (
    CancelCreditOrDebitNoteMixin,
    CancelPurchaseVoucherMixin,
    DeleteRows,
    InputChoiceMixin,
)
from awecounting.libs.nepdate import ad2bs, ad2bs_str
    ...
    def export(self, request):
        if self.is_filtered_by_date():
            input_stream_path = os.path.join(
                settings.BASE_DIR,
                "awecounting/store/spreadsheet_templates/np/sales-book.xlsx",
            )
...
class PurchaseBookViewSet(
    ...
    def export(self, request):
        if self.is_filtered_by_date():
            input_stream_path = os.path.join(
                settings.BASE_DIR,
                "awecounting/store/spreadsheet_templates/np/purchase-book.xlsx",
            )
            input_stream = open(input_stream_path, "rb")
            wb = openpyxl.load_workbook(input_stream)
            ws = wb.active
