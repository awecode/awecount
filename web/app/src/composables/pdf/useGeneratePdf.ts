...
export default function useGeneratePdf(voucherType: VoucherType, onlyBody: boolean, invoiceInfo: object, hideRowQuantity: boolean): string {
  ...
  const companyInfo: Record<string, string | number> = loginStore.companyInfo
  let sameTax = null
  ...
  <div
    ...
    "
  >
    <h4 style="margin: 0; font-size: 1.4rem">${invoiceInfo.status === 'Issued' || invoiceInfo.status === 'Paid' || invoiceInfo.status === 'Partially Paid'
        ? 'TAX INVOICE'
  ...
  <div style="display: flex; justify-content: space-between">
    ...
    <div style="${[2, 3].includes(companyInfo.invoice_template) && invoiceInfo.party_contact_no ? '' : 'display: none;'}">${invoiceInfo.party_contact_no}</div>
