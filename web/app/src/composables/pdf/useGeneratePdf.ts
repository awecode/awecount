...
export default function useGeneratePdf(voucherType: VoucherType, onlyBody: boolean, invoiceInfo: object, hideRowQuantity: boolean): string {
  ...
  <div style="text-align:center; padding-left: 10px;">
    ...
    <div
      ...
      "
    >
      <div style="font-size: 14px;" >VAT No. <strong>${compayInfo.tax_registration_number}</strong></div>
  ...
  <hr style="margin: 20px 0" />`
    ...
    >
      <div style="margin-bottom: 5px;">
        <img src="${compayInfo.logo_url}" alt="Compony Logo" style="height: 70px; max-width: 200px; object-fit: contain; ${compayInfo.logo_url ? '' : 'display: none;'}"/>
      </div>
  ...
  <table style="width: 100%; font-family: Arial, Helvetica, sans-serif; border: 2px solid #b9b9b9;">
    ...
    <th style="white-space: nowrap; padding:5px; border-right: #b9b9b9 solid 2px; border-bottom: #b9b9b9 solid 2px;">H.S. Code</th>
      ...
      <th style="text-align: right; padding:5px; border-bottom: #b9b9b9 solid 2px;">Amount(${compayInfo.config_template === 'np' ? 'NRS' : 'N/A'})</th>
  ...
  >
    <h4 style="margin: 0; font-size: 1.4rem">${
      invoiceInfo.status === 'Issued' || invoiceInfo.status === 'Paid' || invoiceInfo.status === 'Partially Paid' ? 'TAX INVOICE'
  ...
  <div style="display: flex; justify-content: space-between">
    ...
    <div style="${[2, 3].includes(compayInfo.invoice_template) && invoiceInfo.party_contact_no ? '' : 'display: none;'}">${invoiceInfo.party_contact_no}</div>
