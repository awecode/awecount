  ...
  <div class="q-pa-md">
    <q-card>
      <q-card-section class="q-pa-lg">
        ...
        <div>
          <q-input v-model="export_file.password" :type="showPasswordStatus ? 'text' : 'password'" label="Password" input-class="text-body1">
        ...
        <q-btn @click="downloadFile" class="q-mt-md" style="border-radius: 2rem; color: black" label="download" icon-right="download" color="blue" :disable="!export_file.password"></q-btn>
      </q-card-section>
    </q-card>
    <q-card class="q-mt-md">
      <q-card-section class="q-pa-lg">
        ...
        <div>
          ...
          <q-input v-model="import_file.password" :type="showPasswordStatus2 ? 'text' : 'password'" label="Password" input-class="text-body1">
            <template v-slot:before>
              <q-icon name="fa-solid fa-lock" />
            </template>
            <template v-slot:append>
              <q-icon :name="showPasswordStatus2 ? 'mdi-eye' : 'mdi-eye-off'" @click="showPasswordStatus2 = !showPasswordStatus2" class="cursor-pointer" />
            </template>
          </q-input>
        </div>
        <q-btn class="q-mt-md" style="border-radius: 2rem; color: black" label="upload" icon-right="upload" color="green" :disable="!import_file.password"></q-btn>
      </q-card-section>
    </q-card>
  </div>
</template>
