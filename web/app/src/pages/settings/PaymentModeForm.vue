    ...
    <q-card>
      ...
      <q-card-section>
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-md-6">
            <n-auto-complete-v2 v-model="fields.account" label="Account" :options="formDefaults?.collections?.accounts" :staticOption="isEdit ? fields.selected_account_obj : null" endpoint="v1/payment-modes/create-defaults/accounts" option-value="id" option-label="name" :rules="[(v) => !!v || 'Account is required']" filled :error="!!errors.account" :error-message="errors.account" />
          </div>
              ...
              <q-card-section>
                <div class="row q-col-gutter-md">
                  ...
                  <template v-if="fields.transaction_fee_config.type">
                    <div class="col-12 col-md-6">
                      <n-auto-complete-v2 v-model="fields.transaction_fee_account" label="Transaction Fee Account" :options="formDefaults?.collections?.accounts" :staticOption="isEdit ? fields.selected_transaction_fee_account_obj : null" endpoint="v1/payment-modes/create-defaults/accounts" option-value="id" option-label="name" :rules="[(v) => !fields.transaction_fee_config || !!v || 'Fee account is required when fee is enabled']" filled :error="!!errors.transaction_fee_account" :error-message="errors.transaction_fee_account" />
                    </div>
                    ...
                    <template v-if="fields.transaction_fee_config.type === 'slab_based' || fields.transaction_fee_config.type === 'sliding_scale'">
                      <div class="col-12">
                        ...
                        <div v-for="(slab, index) in fields.transaction_fee_config.slabs" :key="index" class="row q-col-gutter-sm q-mb-md">
                          ...
                          <div class="col-12 col-md-3">
                            <q-select v-model="slab.fee_type" :options="slabFeeTypeOptions" label="Fee Type" filled emit-value map-options @update:model-value="onSlabFeeTypeChange(index)" :error="!!validationErrors.transaction_fee_config?.slabs?.[index]?.fee_type" :error-message="validationErrors.transaction_fee_config?.slabs?.[index]?.fee_type" />
                          </div>
                          <div class="col-12 col-md-2">
                            <q-input v-model.number="slab.rate" v-if="slab.fee_type === 'rate'" label="Rate (%)" type="number" step="any" filled min="0" :rules="[(v) => v >= 0 || 'Rate must be greater than or equal to 0', (v) => v <= 100 || 'Rate must be less than or equal to 100']" :error="!!validationErrors.transaction_fee_config?.slabs?.[index]?.rate" :error-message="validationErrors.transaction_fee_config?.slabs?.[index]?.rate" />
                        ...
                        <q-btn color="primary" :label="`Add ${fields.transaction_fee_config.type === 'slab_based' ? 'Slab' : 'Scale'}`" @click="addSlab" class="q-mb-md" />
                      </div>
                    </template>
      ...
      <q-card-actions align="right" class="q-pa-md">
        <q-btn color="red" @click="cancel" label="Cancel" />
