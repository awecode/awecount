...
const onSimilarFetch = () => {
  loading.value = true
  useApi(`/api/company/${route.params.company}/items/similar-items/`)
    .then((data) => {
      if (data.length < 1) {
        $q.notify({
          color: 'red-6',
    ...
    <q-card class="q-ma-md">
      ...
      <q-card-section class="q-pa-lg">
        ...
        <div v-for="(modalValue, index) in modalValueArray" :key="modalValue.index" class="mb-8">
          ...
          <ItemMergeGroup v-model="modalValueArray[index]" :item-options="itemOptions" :selected-items="selectedItems" @remove-group="removeGroup(index)" />
        </div>
