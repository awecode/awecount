        ...
        <q-card-section>
          <div class="row q-col-gutter-md">
            <div class="col-md-6 col-12 row no-wrap">
              <span style="flex-grow: 1"><q-input v-model="fields.invoice_nos" label="For Invoice(s) *" disable :error-message="errors.invoices" :error="!!errors.invoices"></q-input></span>
              <span class="row items-center q-ml-sm" style="flex-grow: 0; flex-shrink: 0"><q-btn icon="add" color="blue" @click="() => (addInoviceModal = !addInoviceModal)"></q-btn></span>
            </div>
            <q-input class="col-md-6 col-12" label="Party" v-model="fields.party_name" disable :error="!!errors?.party_name" :error-message="errors.party_name"></q-input>
          </div>
          <div class="row q-col-gutter-md">
            <DatePicker class="col-md-6 col-12" label="Deposit Date*" v-model="fields.date"></DatePicker>
            <q-select v-model="fields.mode" label="Mode" class="col-12 col-md-6" :error-message="errors.mode" :error="!!errors.mode" :options="['Cheque', 'Cash', 'Bank Deposit']"></q-select>
          </div>
          <div class="row q-col-gutter-md">
            <q-input class="col-md-6 col-12" label="Amount *" type="number" v-model="fields.amount" :error-message="errors.amount" :error="!!errors.amount"></q-input>
            <q-input class="col-md-6 col-12" label="TDS Amount" v-model="fields.tds_amount" :error-message="errors.tds_amount" :error="!!errors.tds_amount" type="number"></q-input>
          </div>
          <div v-if="fields.mode === 'Bank Deposit' || fields.mode === 'Cheque'" class="row q-col-gutter-md">
            <div class="col-md-6 col-12">
              <n-auto-complete-v2 label="Bank Account *" v-model="fields.bank_account" :error-message="errors.bank_account" endpoint="/v1/payment-receipt/create-defaults/bank_accounts" :error="!!errors.bank_account" :staticOption="fields.selected_bank_account_obj" :options="formDefaults.collections?.bank_accounts" option-value="id" option-label="name" map-options emit-value></n-auto-complete-v2>
            </div>
          </div>
          <div v-if="fields.mode === 'Cheque'">
            <div class="row q-col-gutter-md">
              ...
              <q-input class="col-md-6 col-12" label="Cheque Number *" v-model="fields.cheque_number" :error-message="errors.cheque_number" :error="!!errors.cheque_number" type="number"></q-input>
            </div>
          ...
          <div class="q-mt-lg row q-pb-lg flex justify-end">
            <q-btn v-if="checkPermissions('PaymentReceiptCreate') && !isEdit" :loading="loading" @click.prevent="() => onSubmitClick('Issued', fields, submitForm)" color="green" label="Create" type="submit" />
            <q-btn v-if="checkPermissions('PaymentReceiptModify') && isEdit" :loading="loading" @click.prevent="() => onSubmitClick(fields.status, fields, submitForm)" color="green" label="Update" type="submit" />
          </div>
        </q-card-section>
      </q-card>
    </q-card>
      ...
      <q-card style="min-width: min(40vw, 500px)">
        ...
        <q-card-section class="q-mb-md">
          <div class="q-mt-lg q-mx-md">
            <q-input v-model="invoiceFormData.invoice_no" label="Invoice No.*" class="col-12" autofocus type="number" :error="!!errors?.invoice_no" :error-message="errors?.invoice_no"></q-input>
            <div class="q-mx-0 q-my-md">
              <q-checkbox v-model="invoiceFormData.tax_deducted_at_source" label="Tax Deducted at Source?" />
            </div>
            <q-select label="Fiscal Year" v-model="invoiceFormData.fiscal_year" :options="formDefaults.options?.fiscal_years" option-value="id" option-label="name" map-options emit-value :error="!!errors?.fiscal_year" :error-message="errors?.fiscal_year"></q-select>
          </div>
