            ...
            <q-menu>
              <div class="menu-wrapper" style="width: min(500px, 90vw)">
                ...
                <div class="q-ma-sm">
                  ...
                  <div class="q-mx-md">
                    <n-auto-complete-v2 v-model="filters.sales_agent" :fetchOnMount="true" endpoint="v1/sales-agent/choices/" label="Sales Agent" />
                  </div>
                  <div class="q-ma-sm">
                    <MultiSelectChip :options="['Issued', 'Cleared', 'Cancelled']" v-model="filters.status" />
                  </div>
                  <div class="q-ma-sm">
                    <MultiSelectChip label="Mode(s):" :options="['Cheque', 'Cash', 'Bank Deposit']" v-model="filters.mode" />
                  </div>
                </div>
                <div class="q-mx-md">
        ...
        <q-td :props="props" style="padding: 0">
          <div class="row align-center text-subtitle2 text-grey-8" style="padding: 8px 8px 8px 16px">
            <span v-for="invoice in props.row.invoices" :key="invoice.id" style="margin-right: 8px">
              <router-link v-if="checkPermissions('SalesView')" :to="`/sales-voucher/${invoice.id}/view`" style="text-decoration: none; display: flex; align-items: center" class="text-blue">#{{ invoice.voucher_no }}</router-link>
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <div class="row q-gutter-x-md">
            <q-btn v-if="checkPermissions('PaymentReceiptView')" color="blue" label="View" class="q-py-none q-px-md font-size-sm l-view-btn" style="font-size: 12px" :to="`/payment-receipt/${props.row.id}/view`" />
          </div>
        </q-td>
      </template>
    </q-table>
