...
<template>
  <div class="q-pa-md">
    <div class="row q-gutter-x-md justify-end">
      <q-btn
        v-if="checkPermissions('quotations.create')"
        class="add-btn"
        color="green"
        icon-right="add"
        label="New Quotation"
    ...
    >
          ...
          <q-btn class="f-open-btn" icon="mdi-filter-variant">
            <q-menu>
              <div class="menu-wrapper" style="width: min(550px, 90vw)">
                ...
                <div class="q-ma-sm">
                  <div class="q-mb-sm">
                    <q-checkbox v-model="filters.is_expired" label="Is Expired?" :false-value="null" />
                  </div>
                  <div class="q-ma-sm">
                    <MultiSelectChip v-model="filters.status" :options="['Draft', 'Generated', 'Sent', 'Converted']" />
                  </div>
                </div>
                <div class="q-mx-md">
      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          <div class="row q-gutter-x-md justify-start">
            <q-btn
              v-if="checkPermissions('quotations.read')"
      ...
      <template #body-cell-number="props">
        <q-td style="padding: 0" :props="props">
          <span v-if="checkPermissions('quotations.read')" data-testid="number">
            <router-link
              v-if="checkPermissions('quotations.read') && props.row.number"
              class="text-blue"
          ...
          <span v-else data-testid="number" style="display: flex; align-items: center; height: 100%; padding: 8px 8px 8px 16px">
            {{ props.row.number }}
          </span>
        </q-td>
      </template>
