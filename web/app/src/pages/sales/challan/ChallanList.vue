      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          <div class="row q-gutter-x-md">
            <q-btn v-if="checkPermissions('challan.modify')" color="orange" label="Edit" class="q-py-none q-px-md font-size-sm l-edit-btn" style="font-size: 12px" :to="`/${$route.params.company}/challan/${props.row.id}/`" />
          </div>
        </q-td>
      </template>
      <template #body-cell-voucher_no="props">
        <q-td :props="props">
          <span v-if="props.row.voucher_no">
            <router-link v-if="checkPermissions('challan.modify')" :to="`/${$route.params.company}/challan/${props.row.id}/`" style="font-weight: 500; text-decoration: none" class="text-blue">
