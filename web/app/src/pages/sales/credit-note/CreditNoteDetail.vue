...
export default {
  setup() {
    ...
    const submitChangeStatus = (id, status) => {
      ...
      let body = null
      if (status === 'Paid') {
        endpoint = `/api/company/${route.params.company}/credit-note/${id}/mark_as_paid/`
        body = { method: 'POST' }
      } else if (status === 'Cancelled') {
        endpoint = `/api/company/${route.params.company}/credit-note/${id}/cancel/`
        body = { method: 'POST' }
      }
    ...
    const onPrintclick = (noApiCall = false) => {
      if (!noApiCall) {
        const endpoint = `/api/company/${route.params.company}/credit-note/${fields.value.id}/log-print/`
  ...
  created() {
    const endpoint = `/api/company/${this.$route.params.company}/credit-note/${this.$route.params.id}/details/`
    ...
    <div v-if="fields" class="q-px-lg q-pb-lg row justify-between q-gutter-x-md q-mt-md">
      <div v-if="fields?.status !== 'Cancelled'" class="row q-gutter-x-md q-mb-md">
        <q-btn
          ...
          icon="edit"
          label="Edit"
          :to="`/${$route.params.company}/sales/credit-notes/${fields.id}/edit`"
        />
        <q-btn
      ...
      <div v-if="fields?.status !== 'Cancelled'" class="row q-gutter-x-md q-gutter-y-md q-mb-md">
        ...
        <q-btn
          ...
          :to="`/${$route.params.company}/sales/credit-notes/${fields.id}/journal-entries`"
        />
      </div>
