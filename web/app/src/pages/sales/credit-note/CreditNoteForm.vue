    ...
    <q-card>
        ...
        <q-card-section>
          ...
          <div class="row q-col-gutter-xl">
            ...
            <div class="col-md-6 col-12">
              <n-auto-complete-v2 v-model="fields.payment_mode" label="Payment Mode *" :error-message="errors?.payment_mode" endpoint="/v1/credit-note/create-defaults/payment_modes" :error="!!errors?.payment_mode" :options="modeOptionsComputed" :staticOption="isEdit ? fields.selected_payment_mode_obj : formDefaults.options?.default_payment_mode_obj" data-testid="mode-input">
      ...
      <div class="row q-px-lg">
        ...
        <div class="col-12 col-md-6 row justify-between">
          ...
          <q-input v-if="fields.sales_agent?.name" label="Sales Agent" class="col-8" disable v-model="fields.sales_agent.name"></q-input>
        </div>
      </div>

      <div class="q-pr-md q-pb-lg q-mt-md row justify-end q-gutter-x-md">
        <q-btn v-if="checkPermissions('CreditNoteCreate') && (!isEdit || (isEdit && fields.status === 'Draft'))" :loading="loading" @click.prevent="() => onSubmitClick('Draft')" color="orange" :label="isEdit ? 'Update Draft' : 'Save Draft'" :disabled="!(fields.invoices && fields.invoices.length > 0)" type="submit" />
        <q-btn
          @click.prevent="
            () =>
              onSubmitClick(
...
export default {
  ...
  setup(props, { emit }) {
    ...
    useMeta(() => {
      return {
        title: (formData.isEdit?.value ? 'Credit Note Update' : 'Credit Note Add') + ' | Awecount',
      }
    })
