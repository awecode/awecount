  ...
  <div class="q-pa-md">
    ...
    <q-table v-model:pagination="pagination" title="Accounts" :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" row-key="id" class="q-mt-md" :rows-per-page-options="[20]" @request="onRequest">
      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          <q-btn v-if="checkPermissions('party.modify')" color="orange-6" class="q-py-none q-px-md font-size-sm q-mr-sm l-view-btn" style="font-size: 12px" label="edit" :to="`/${$route.params.company}/party/${props.row.id}/`" />
          <q-btn v-if="checkPermissions('party.view')" color="blue" class="q-py-none q-px-md font-size-sm l-edit-btn" style="font-size: 12px" label="Account" :to="`/${$route.params.company}/parties/account/${props.row.id}/`" />
        </q-td>
      </template>
      <template #body-cell-name="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('party.view')" :to="`/${$route.params.company}/parties/account/${props.row.id}/`" style="font-weight: 500; text-decoration: none" class="text-blue block">
            {{ props.row.name }}
          </router-link>
          <span v-else>
            {{ props.row.name }}
