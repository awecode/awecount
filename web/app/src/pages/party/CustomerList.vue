    ...
    <q-table v-model:pagination="pagination" title="Accounts" :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" row-key="id" class="q-mt-md" :rows-per-page-options="[20]" @request="onRequest">
      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn v-if="checkPermissions('party.view')" color="blue" class="q-py-none q-px-md font-size-sm l-view-btn" style="font-size: 12px" label="Account" :to="`/${$route.params.company}/parties/account/${props.row.id}/`" />
        </q-td>
      </template>
