<template>
    ...
    <q-table :rows="rows" :columns="columns" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md" :rows-per-page-options="[20]">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn v-if="checkPermissions('ReconciliationStatementView') && props.row.reconciled_rows < props.row.total_rows" color="green" class="q-py-none q-px-md font-size-sm q-mr-md l-view-btn" style="font-size: 12px" label="Reconcile" :to="`/bank-reconciliation/reconcile/?account_id=${props.row.account.id}&start_date=${props.row.start_date}&end_date=${props.row.end_date}`" />
          <q-btn v-else-if="checkPermissions('ReconciliationStatementView')" color="blue" class="q-py-none q-px-md font-size-sm q-mr-md l-view-btn" style="font-size: 12px" label="Reconciled" disable />
          <q-btn v-if="checkPermissions('ReconciliationStatementDelete')" color="red" class="q-py-none q-px-md font-size-sm q-mr-md l-view-btn" style="font-size: 12px" label="Delete" @click="deleteStatement(props.row.id)" />
        </q-td>
      </template>
      <template v-slot:body-cell-category="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('CategoryModify')" style="font-weight: 500; text-decoration: none" class="text-blue" :to="`/account-category/${props.row.category.id}/`">{{ props.row.category.name }}</router-link>
          <span v-else>{{ props.row.category.name }}</span>
        </q-td>
      </template>
      <template v-slot:body-cell-account="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('AccountView')" :to="`/account/${props.row.account.id}/view/?start_date=${props.row.start_date}&end_date=${props.row.end_date}`" style="font-weight: 500; text-decoration: none" class="text-blue">
      ...
      <template v-slot:body-cell-rows_update="props">
        <q-td :props="props">
          <div v-if="props.row.has_updated_rows" class="flex gap-4 items-center bg-yellow-500 p-2 rounded-md w-fit">
            ...
            <q-btn color="yellow" class="text-black q-py-none q-px-md font-size-sm q-mr-md l-view-btn" style="font-size: 12px" label="View Updated Rows" @click="fetchUpdatedTransactions(props.row.id)" />
          </div>
        </q-td>
      </template>
      <!-- no-data -->
  ...
  <q-dialog no-shake v-model="statementPrompt">
    <q-card style="min-width: 350px" class="p-5 space-y-3">
      <div class="text-xl text-gray-700 font-bold">Statement Upload</div>
      <q-form @submit="submitStatement">
        ...
        <q-file v-if="selectedDateFormat" bottom-slots v-model="statementSheet" label="Statement Document" counter max-files="1" accept=".xlsx, .xls" class="q-mb-md" @update:model-value="parseExcelFile">
          ...
          <template v-slot:hint>
            ...
            <div v-else-if="toHaveHeaders.find((header) => !statementHeaders.has(header))" class="text-red-600 text-xs">Couldn't find header columns: {{ toHaveHeaders.filter((header) => !statementHeaders.has(header)).join(', ') }}</div>
            <div v-else-if="statementData.length" class="text-green-600 text-xs">{{ statementData.length }} rows parsed</div>
          </template>
        </q-file>
        ...
        <div class="text-right !mt-6">
          <q-btn type="submit" :loading="isLoading" label="Upload" color="green" :disable="!statementAccount || !statementSheet || isStatementProcessing || toHaveHeaders.find((header) => !statementHeaders.has(header))" />
        </div>
      </q-form>
    </q-card>
  </q-dialog>
    ...
    <q-card style="min-width: 900px">
      <div class="bg-white shadow-lg rounded-lg overflow-hidden border">
        <div v-if="updatedData?.results.length" class="p-4 bg-gray-50 max-h-[800px] overflow-y-auto matched-transactions">
          <div class="text-xl font-semibold text-gray-700 mb-6">Updated Transactions</div>
          <q-infinite-scroll ref="infiniteScroll" @load="loadMore" :offset="250" scroll-target=".matched-transactions">
            <div v-for="data in updatedData?.results" :key="data.statement_transactions[0].id" class="border-b-2 mb-5 pb-5">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div v-else class="flex flex-col items-center justify-center bg-white border rounded-lg shadow-sm overflow-hidden grow text-gray-500 text-center p-4 mb-9">Transactions might have been deleted</div>
              </div>
              <div class="flex justify-end space-x-3">
                <button v-if="data.system_transactions.length" @click="updateTransactions(data)" class="px-3 py-1.5 bg-green-500 text-white text-sm rounded-md hover:bg-green-600 transition-colors">Reconcile</button>
