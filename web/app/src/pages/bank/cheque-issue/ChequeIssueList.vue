<template>
  <div class="q-pa-md">
    <div class="flex gap-4 justify-end">
      ...
      <q-btn v-if="checkPermissions('ChequeIssueCreate')" color="green" to="/cheque-issue/add/" label="New Cheque Issue" class="add-btn" icon-right="add" />
    </div>
    ...
    <q-table :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md" :rows-per-page-options="[20]">
      ...
      <template v-slot:body-cell-issued_to="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('ChequeIssueModify')" style="font-weight: 500; text-decoration: none" class="text-blue" :to="`/cheque-issue/${props.row.id}/`">{{ props.row.issued_to || props.row.party_name }}</router-link>
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          <q-btn v-if="checkPermissions('ChequeIssueModify')" label="Edit" color="orange-6" class="q-py-none q-px-md font-size-sm l-view-btn" style="font-size: 12px" :to="`/cheque-issue/${props.row.id}/`" />
        </q-td>
      </template>
    </q-table>
  </div>
