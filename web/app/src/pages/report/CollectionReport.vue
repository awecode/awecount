    ...
    <q-card-section>
      <div>
        <div class="flex gap-x-8 gap-y-2 items-center">
          ...
          <q-btn :disable="!fields.start_date && !fields.end_date ? true : false" color="green" label="fetch" @click="fetchData" class="f-submit-btn"></q-btn>
        </div>
      </div>
      <div v-if="reportData">
        <q-card class="q-mt-md">
          <div>
            <div class="row q-pa-md" style="border-bottom: 2px solid lightgray">
              <div class="col-6 text-weight-medium text-grey-8">Sales Agent</div>
              <div class="col-6 text-weight-medium text-grey-8">Amount</div>
            </div>
            <div v-for="row in reportData.values" :key="row.invoices__sales_agent_id" class="row q-pa-md" style="border-bottom: 2px solid lightgray">
              <div class="col-6 text-grey-8">
                <router-link :to="`/payment-receipt/list?start_date=${fields.start_date}&end_date=${fields.end_date}&sales_agent=${row.invoices__sales_agent_id}`" target="_blank" style="text-decoration: none" class="text-blue">
