      ...
      <div>
        <div class="flex flex-col gap-10">
          <div class="flex gap-12 col-6">
            ...
            <q-checkbox v-model="viewTransactionOnly" label="Show Transaction only" class="mr-10" @click="filterTransactionData" />
          </div>
            ...
            <div class="flex flex-col gap-2">
              <div class="flex flex-col gap-3">
                ...
                <div v-if="salesData?.results">
                  <q-table :rows="salesData?.results" :columns="salesColumns" flat bordered :rows-per-page-options="[12]" v-model:pagination="salesPagination" @request="onSalesRequest">
                    <template v-slot:body-cell-voucher_no="props">
                      <q-td :props="props">
                        <router-link target="_blank" style="font-weight: 500; text-decoration: none" class="text-blue" :to="`/sales-voucher/${props.row.id}/view`">{{ props.row.voucher_no }}</router-link>
                      </q-td>
                    </template>
                  </q-table>
                </div>
            ...
            <div class="flex flex-col gap-2">
              <div class="flex flex-col gap-3">
                ...
                <div v-if="purchaseData?.results" class="flex flex-col gap-3">
                  <q-table :rows="purchaseData?.results" :columns="purchaseColumns" flat bordered :rows-per-page-options="[20]" v-model:pagination="purchasePagination" @request="onPurchaseRequest">
                    <template v-slot:body-cell-voucher_no="props">
                      <q-td :props="props">
                        <router-link target="_blank" style="font-weight: 500; text-decoration: none" class="text-blue" :to="`/purchase-voucher/${props.row.id}/view`">{{ props.row.voucher_no }}</router-link>
                      </q-td>
                    </template>
                  </q-table>
                </div>
        ...
        <div class="mt-4">
          <div v-if="data">
            <q-table :rows="data" :columns="columns" flat bordered :rows-per-page-options="[20]" :filter="searchQuery" :filter-method="filterData">
              <template v-slot:top>
              ...
              <template v-slot:body-cell-action="props">
                <q-td>
                  <RouterLink style="text-decoration: none; display: flex; align-items: center; height: 100%; padding: 8px 8px 8px 16px" class="text-blue-6" target="_blank" :to="`/account/${props.row.account.id}/view/?start_date=${date}&end_date=${date}`">View Transactions</RouterLink>
                </q-td>
              </template>
            </q-table>
          </div>
