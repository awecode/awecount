<template>
  <div class="q-pa-md">
    <div class="flex gap-4 justify-end">
      ...
      <q-btn v-if="checkPermissions('PurchaseVoucherCreate')" color="green" to="/purchase-voucher/add/" label="New Purchase" icon-right="add" class="add-btn" />
    </div>
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <div class="row q-gutter-x-md items-center" v-if="checkPermissions('PurchaseVoucherView')">
            <q-btn color="blue" label="View" :to="`/purchase-voucher/${props.row.id}/view`" class="q-py-none q-px-md font-size-sm l-view-btn" style="font-size: 12px" data-testid="view-btn" />
          </div>
        </q-td>
        <!-- TODO: add modals -->
      </template>
      <template v-slot:body-cell-voucher_no="props">
        <q-td :props="props" style="padding: 0">
          <span v-if="props.row.voucher_no">
            <router-link v-if="checkPermissions('PurchaseVoucherView')" :to="`/purchase-voucher/${props.row.id}/view`" style="font-weight: 500; text-decoration: none; display: flex; align-items: center; height: 100%; padding: 8px 8px 8px 16px" class="text-blue" data-testid="voucher-no">
              {{ props.row.voucher_no }}
            </router-link>
            <span v-else style="display: flex; align-items: center; height: 100%; padding: 8px 8px 8px 16px" data-testid="voucher-no">
              {{ props.row.voucher_no }}
            </span>
          </span>
        </q-td>
    ...
    <XLSImport v-model:show-import-modal="showImportModal" :required-columns="importRequiredColumns" endpoint="/v1/purchase-vouchers/import/" title="Import Purchase Vouchers" help-text="Upload a .xlsx file to import purchase invoices" sample-file-url="/files/purchase-invoices.xlsx" />
  </div>
</template>
