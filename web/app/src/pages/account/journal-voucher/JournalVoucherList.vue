<template>
  <div class="q-pa-md">
    <div class="row justify-between">
      <div></div>
      <q-btn color="green" to="/journal-voucher/add/" label="New Journal Voucher" class="q-ml-lg" icon-right="add" />
    </div>
    ...
    <q-table title="Accounts" :rows="rows" :columns="columns" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn v-if="props.row.status != 'Cancelled'" icon="edit" color="amber" dense flat :to="`/journal-voucher/${props.row.id}/edit/`" />
