<template>
  <div class="q-pa-md">
    <div class="row justify-between">
      <div></div>
      <q-btn color="green" to="/account/add/" label="New Account" class="q-ml-lg" icon-right="add" />
    </div>
    ...
    <q-table title="Accounts" :rows="rows" :columns="columns" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn icon="edit" color="amber" dense flat :to="`/account/${props.row.id}/edit/`" />
          <!-- <q-btn
            icon="delete"
            color="red"
            dense
      ...
      <template v-slot:body-cell-category="props">
        <q-td :props="props">
          <router-link :to="`/account/category/${props.row.category.id}/edit/`">{{ props.row.category.name }}</router-link>
        </q-td>
      </template>
    </q-table>
  </div>
