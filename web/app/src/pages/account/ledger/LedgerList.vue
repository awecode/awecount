...
<template>
  <div class="q-pa-md">
    <div class="row justify-end">
      <q-btn v-if="checkPermissions('account.create')" color="green" :to="`/${$route.params.company}/account/create/`" label="New Account" class="add-btn" icon-right="add" />
    </div>
    ...
    <q-table v-model:pagination="pagination" :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" row-key="id" class="q-mt-md" :rows-per-page-options="[20]" @request="onRequest">
            ...
            <q-menu>
              <div class="menu-wrapper" style="width: min(300px, 90vw)">
                ...
                <div class="q-ma-sm">
                  ...
                  <div class="q-mx-sm">
                    <n-auto-complete-v2 v-model="filters.category" :fetch-on-mount="true" :endpoint="`/api/company/${$route.params.company}/categories/choices/`" label="Category" />
                  </div>
                </div>
      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn v-if="checkPermissions('account.view')" color="blue" class="q-py-none q-px-md font-size-sm q-mr-md l-view-btn" style="font-size: 12px" label="View" :to="`/${$route.params.company}/account/${props.row.id}/view/`" />
          <q-btn v-if="checkPermissions('account.modify')" label="Edit" color="orange-6" class="q-py-none q-px-md font-size-sm l-edit-btn" style="font-size: 12px" :to="`/${$route.params.company}/account/${props.row.id}/edit/`" />
        </q-td>
      </template>
      <template #body-cell-category="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('category.modify')" style="font-weight: 500; text-decoration: none" class="text-blue" :to="`/${$route.params.company}/account-category/${props.row.category.id}/`">
      ...
      <template #body-cell-name="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('account.view')" :to="`/${$route.params.company}/account/${props.row.id}/view/`" style="font-weight: 500; text-decoration: none" class="text-blue">
            {{ props.row.name }}
          </router-link>
