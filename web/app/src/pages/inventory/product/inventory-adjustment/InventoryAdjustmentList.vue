<template>
  <div class="q-pa-md">
    <div class="row justify-end q-gutter-md" v-if="checkPermissions('InventoryAdjustmentVoucherCreate')">
      <q-btn color="green" class="add-btn" to="/items/inventory-adjustment/add" label="Add Inventory Adjustment Voucher" icon-right="add" />
    </div>
    <q-table title="Income Items" :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md" :rows-per-page-options="[20]">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          <q-btn v-if="checkPermissions('InventoryAdjustmentVoucherView')" color="blue-6" class="q-py-none q-px-md font-size-sm q-mr-sm l-edit-btn" style="font-size: 12px" label="View" :to="`/items/inventory-adjustment/${props.row.id}/view`" />
          <q-btn v-if="checkPermissions('InventoryAdjustmentVoucherModify')" color="orange-6" class="q-py-none q-px-md font-size-sm q-mr-sm l-edit-btn" style="font-size: 12px" label="edit" :to="`/items/inventory-adjustment/${props.row.id}/`" />
        </q-td>
      </template>

      <template v-slot:body-cell-voucher_no="props">
        <q-td :props="props">
          <router-link v-if="checkPermissions('InventoryAdjustmentVoucherModify')" :to="`/items/inventory-adjustment/${props.row.id}/`" style="font-weight: 500; text-decoration: none" class="text-blue">
