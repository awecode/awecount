<template>
  <div class="q-pa-md">
    <div class="row justify-end q-gutter-md" v-if="checkPermissions('BillOfMaterialCreate')">
      <q-btn color="green" class="add-btn" to="/items/bill-of-material/add" label="Add Bill of Material" icon-right="add" />
    </div>
    <q-table title="Income Items" :rows="rows" :columns="newColumn" :loading="loading" :filter="searchQuery" v-model:pagination="pagination" row-key="id" @request="onRequest" class="q-mt-md" :rows-per-page-options="[20]">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          <q-btn v-if="checkPermissions('BillOfMaterialModify')" color="orange-6" class="q-py-none q-px-md font-size-sm q-mr-sm l-edit-btn" style="font-size: 12px" label="edit" :to="`/items/bill-of-material/${props.row.id}/`" />
        </q-td>
      </template>
