    ...
    <q-card>
      ...
      <q-card class="q-mx-lg q-pt-md">
        <q-card-section>
          <div class="row q-col-gutter-md">
            <div v-if="!isEdit" class="col-12 col-md-6">
              <n-auto-complete-v2 v-model="fields.item_id" :options="formDefaults.collections?.items" label="Item *" :error="errors?.item_id" :modalComponent="ItemAdd" endpoint="/v1/item-opening-balance/create-defaults/items" />
            </div>
        ...
        <div class="text-right q-pr-md q-pb-lg flex gap-4 justify-right">
          <q-btn v-if="isEdit && checkPermissions('AccountOpeningBalanceDelete')" :loading="loading" @click.prevent="isDeleteOpen = true" icon="delete" color="red" label="Delete" type="submit" />
          <q-btn v-if="!isEdit && checkPermissions('AccountOpeningBalanceCreate')" :loading="loading" @click.prevent="submitForm" color="green" label="Create" type="submit" />
          <q-btn v-if="isEdit && checkPermissions('AccountOpeningBalanceModify')" :loading="loading" @click.prevent="submitForm" color="green" label="Update" type="submit" />
        </div>
      </q-card>
...
export default {
  ...
  setup(props, context) {
    ...
    const onDeleteClick = (id) => {
      useApi(`/v1/item-opening-balance/${id}/`, { method: 'DELETE' })
        .then(() => {
