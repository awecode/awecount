...
<template>
  <div>
    ...
    <div class="row no-wrap q-gutter-x-md q-py-sm items-center">
      ...
      <div class="row q-col-gutter-md" style="flex-grow: 1">
        <div class="col-2">
          ...
          <q-select v-model="voucher.type" :options="['Dr', 'Cr']" label="Type" error-message="" :error="false" @update:model-value="onSwitchDrCr" />
        ...
        <div class="col-2">
          <q-input v-model.Number="voucher.dr_amount" type="number" min="0" label="Dr. Amount" class="col-6" :disable="voucher.type == 'Cr' || voucher.type == null" :error-message="props.errors?.mismatch" :error="!!props.errors?.mismatch" @focusout="focusOut" />
        </div>
        <div class="col-2">
          <q-input v-model.Number="voucher.cr_amount" type="number" min="0" label="Cr. Amount" class="col-6" :disable="voucher.type == 'Dr' || voucher.type == null" :error-message="props.errors?.mismatch" :error="!!props.errors?.mismatch" @focusout="focusOut" />
        </div>
        <div class="row no-wrap items-center justify-center" style="flex-grow: 1">
          <q-btn flat class="q-pa-sm focus-highLight" color="transparent" @click.prevent="() => (openDescription = !openDescription)">
            <q-icon name="mdi-arrow-expand" size="20px" color="green" class="cursor-pointer">
              <q-tooltip>Expand</q-tooltip>
            </q-icon>
          </q-btn>
          <q-btn flat class="q-pa-sm focus-highLight" color="transparent" @click="deleteVoucher">
    ...
    <div v-if="openDescription" class="row q-pb-md q-mx-xl">
      <q-input v-model="voucher.description" type="textarea" autogrow label="Description" class="col-12" />
    </div>
  </div>
</template>
