...
export default {
  ...
  setup(props, { emit }) {
    ...
    const calculateNet = (obj) => {
      ...
      if (netAmount >= 0) return Number.parseFloat(netAmount.toFixed(2))
      return `(${Number.parseFloat(netAmount.toFixed(2)) * -1})`
    }
    ...
    watch(
      ...
      (newValue) => {
        ...
        if (showTotalObject.value && showTotalObject.value.length > 0) {
          showTotalObject.value.forEach((totalobj, totalIndex) => {
            if (!computedTotal[totalIndex]) computedTotal[totalIndex] = { ...totalObjectFormat }
            fieldsArray.forEach((field) => {
              computedTotal[totalIndex][field] = computedTotal[totalIndex][field] + totalobj[field]
            })
