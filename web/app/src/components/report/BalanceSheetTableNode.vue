<template>
  <template v-if="!props.config.hide_zero_transactions">
    ...
    <template v-if="newTotalObjArray && newTotalObjArray.length > 0">
      <tr :class="expandAccountsProps && (!props.config.hide_categories || props.root) ? '' : 'hidden'">
        ...
        <td class="text-blue-6" :class="props.root ? 'text-weight-bold' : ''">
          <span v-for="num in level" :key="num">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
          <span style="display: inline-block; width: 40px; margin-left: -5px" v-if="!props.config.hide_categories">
          ...
          <RouterLink style="text-decoration: none" target="_blank" :to="`/account/?has_balance=true&category=${item.id}`" class="text-blue-6">{{ item.name }}</RouterLink>
        </td>
        <template v-for="(newTotalObj, index) in newTotalObjArray" :key="index">
    ...
    <template v-if="!(newTotalObjArray && newTotalObjArray.length > 0) && checkZeroTrans(showTotalObject) && !props.config.hide_categories">
  ...
  <template v-for="(activeObject, key) in activeObjectComputed" :key="key">
    ...
    >
      ...
      <td class="text-blue-6 text-italic">
        ...
        <RouterLink target="_blank" style="text-decoration: none" :to="`/account/${activeObject.account_id}/view/`" class="text-blue-7 text-italic text-weight-regular">{{ activeObject.name }}</RouterLink>
      </td>
  ...
  <template v-if="item.children && item.children.length">
    <BalanceSheetTableNode v-for="(child, index) in item.children" :key="child.id" :item="child" :index="index" :level="props.level + 1" :accounts="props.accounts" :category_accounts="props.category_accounts" @updateTotal="onUpdateTotal" :config="props.config" :expandAccountsProps="expandAccountsProps && expandStatus" :isAsset="props.isAsset"></BalanceSheetTableNode>
  </template>
</template>
