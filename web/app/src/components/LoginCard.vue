    ...
    <div class="full-width q-px-md">
      <q-form class="text-sm" @submit="onLoginSubmit">
        ...
        <q-input v-model="username" label="Email" input-class="text-body1"
          ...
          :error-message="errorMessage ? (errorMessage.email ? errorMessage.email[0] : null) : null">
          <template v-slot:before>
            <q-icon name="fa-solid fa-envelope" />
          </template>
        </q-input>
        <q-input v-model="password" :type="showPasswordStatus ? 'text' : 'password'" label="Passowrd"
          input-class="text-body1" :error="Boolean((errorMessage?.password) ? true : false)"
          :error-message="errorMessage ? (errorMessage.password ? errorMessage.password[0] : null) : null">
          <template v-slot:before>
            <q-icon name="fa-solid fa-lock" />
          </template>
          <template v-slot:append>
            <q-icon :name="showPasswordStatus ? 'fa-solid fa-eye' : 'fa-thin fa-eye'"
...
const onLoginSubmit = async () => {
  ...
  await useApi('v1/auth/login/', {
    ...
    $q.notify({
      color: 'red-6',
      message: err.status == '500' ? 'Server Error! Please contact us with the problem.' : 'Please provide valid credentials.',
      icon: 'report_problem',
      position: 'top-right'
    })
  })
