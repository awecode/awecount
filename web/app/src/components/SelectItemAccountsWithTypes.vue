<template>
  <div class="grid lg:grid-cols-2 lg:gap-4">
    <div>
      <q-select class="q-full-width" :label="`${label} A/C Options`" option-value="value" option-label="label" map-options emit-value v-model="type" :options="account_types" :error="false" @update:model-value="onTypeChange" />
    </div>
    <div>
      <n-auto-complete-v2 v-if="type === 'existing'" class="q-full-width" :label="`${label} Account`" v-model="modalValue" :options="props.options" :endpoint="endpointLabelMap[props.label]" :modal-component="checkPermissions('account.create') ? LedgerForm : null" :error="error" :staticOption="injectOption" />
      ...
      <q-select v-else :label="`${label} Account`" option-value="id" option-label="name" map-options emit-value v-model="modalValue" disable :options="globalAccountObjComputed.id ? [globalAccountObjComputed] : []" :error="!!error" :error-message="error"></q-select>
    </div>
  </div>
</template>
