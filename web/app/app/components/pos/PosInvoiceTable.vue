    ...
    const totalDataComputed = computed(() => {
      ...
      modalValue.value.forEach((item, index) => {
        ...
        if (data.sameScheme !== false && rowTaxObj) {
          if (data.sameScheme === null && rowTaxObj && rowTaxObj.rate != 0) {
        ...
        if (rowTaxObj) {
          ...
          if (props.mainDiscount.discount_type === 'Amount') {
            rowTax = (rowTotal - (rowDiscount || 0) - props.mainDiscount.discount * (rowTotal / data.addTotal)) * (rowTaxObj.rate / 100 || 0)
          } else {
            rowTax = (rowTotal - (rowDiscount || 0) - mainDiscountAmount) * (rowTaxObj.rate / 100 || 0)
          }
